...
SECRET_KEY = "secreto_super_seguro_taller_dwm" # En producción usar variable de entorno
ALGORITHM = "HS256"
...
# Dependencia: Valida que el usuario tenga un token real
...
# Dependencia: Solo deja pasar si es Admin
async def get_current_admin(current_user: dict = Depends(get_current_user)):
    if current_user["role"] != "admin":
        logger.warning(f"Acceso admin denegado a: {current_user['email']}")
...
def register_user(user: User):
    ...
    # ENCRIPTAR CONTRASEÑA (Aquí ocurre la magia de seguridad)
    user_dict["password"] = get_password_hash(user.password)
    
    # Asignar rol admin si es correo corporativo
...
def login_user(user: User):
    ...
    # Verificar contraseña HASH (Ya no es texto plano)
    if not db_user or not verify_password(user.password, db_user["password"]):
        logger.warning(f"Login fallido: {user.email}")
...
def create_order(order: Order, current_user: dict = Depends(get_current_user)):
    ...
    # Guardar Orden (Usando email del token para evitar suplantación)
...
def get_orders_by_user(email: str, current_user: dict = Depends(get_current_user)):
    # Seguridad: Un usuario solo puede ver SUS propias órdenes
    if current_user["email"] != email and current_user["role"] != "admin":
        raise HTTPException(403, "No tienes permiso para ver estas órdenes")
